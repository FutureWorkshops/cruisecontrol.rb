...
EOF
  ...
  def output
    @output ||= contents_for_display(build_log, true)
  end
  ...
  def contents_for_display(file, reverse_mode=false)
    return '' unless file.file? && file.readable?
    reverse_mode ? ("[...]\n" + `tail -c #{Configuration.max_file_display_length} #{file}`.lines.to_a[2..-1].join) : file.read(Configuration.max_file_display_length)
  end
